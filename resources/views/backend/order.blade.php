...
<script type="text/javascript">
    ...
    // $('.add_items').click(function(event){
        ...
        $.ajax({
            type:"POST",
            dataType:"html",
            url: "{{URL::route('admin-getItemList')}}",
            data: {
                _token: token,
                item_id: item_id,
                quantity: quantity,
...
<script type="text/javascript">
        ...
        $.ajax({
            type:"POST",
            dataType:"html",
            url: "{{URL::route('admin-getItemCalculation')}}",
            data: {
                _token: token,
                total: total,
            },
...
<script type="text/javascript">
    $(document).on("keyup","#grand_total",function() {
        var discount = $('#discount').val();
        var due = $('#due').val();
        var amount = $('#amount').val();
        $('#grand_total').val(amount-discount-due);
        if(amount == '0'){
            $('.print').addClass('disabled');
        }
        else{
